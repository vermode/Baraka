...
/**
 * The single site header used on every page.
 * Common controls (language, theme, map) are always shown. The right-hand
 * cluster is auth-aware: signed-in users get the `UserMenu` (account, name,
 * notifications); signed-out visitors get the Login / Sign up call-to-actions.
 */
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-20 max-w-screen-2xl items-center justify-between px-4 sm:px-8">
        ...
        {/* Logo — links to the dashboard when signed in, otherwise the landing page. */}
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {/* Signed in: account cluster. Signed out: auth CTAs + mobile nav toggle. */}
          {user ? (
            <UserMenu />
          ) : (
            <>
              ...
              <Link href="/signup">
                <Button className="hidden sm:inline-flex" data-testid="btn-signup">
                  {t('nav.signup')}
                </Button>
              </Link>
      ...
      {/* Mobile Menu — landing nav for signed-out visitors only. */}
      {!user && isMobileMenuOpen && (
        <div className="sm:hidden border-t bg-background px-4 py-4 flex flex-col gap-2">
          ...
          <Link href="/signup" className="w-full">
            <Button className="w-full mt-2" data-testid="btn-mobile-signup">
              {t('nav.signup')}
            </Button>
          </Link>
        </div>
